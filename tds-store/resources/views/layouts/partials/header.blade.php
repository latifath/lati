        ...
        <div class="col-lg-6 d-none d-lg-block">
            <div class="d-inline-flex align-items-center" style="{{ couleur_text_3() }}; font-size: 15px;">
                ...
                <span class="text-nowrap">
                    <strong><i class="fa fa-phone" style="{{couleur_text_1() }}"></i></strong>
                    <a href="tel:+370 615 85560"  style="{{ couleur_text_3() }}; font-size: 15px;">+370 615 85560</a>
                </span>
            ...
            <div class="d-inline-flex align-items-center">
                ...
                @auth
                    <form method="POST" action="{{ route('logout') }}" x-data>
                        ...
                        <x-jet-responsive-nav-link href="{{ route('logout') }}" @click.prevent="$root.submit();" style="{{ couleur_text_3() }}; font-size: 15px;">
                            Déconnexion
                    ...
                    @else
                        <a href="{{ route('root_auth_user_login') }}" class="nav-item nav-link  p-0 " style="{{ couleur_text_3() }}; font-size: 15px;">Connexion</a>

                        @if (Route::has('register'))
                            <span class="text-muted px-1">|</span>

                            <a href="{{ route('root_auth_user_register') }}" class="nav-item nav-link  p-0"  style="{{ couleur_text_3() }}; font-size: 15px;">Inscription</a>
                        @endif
                @endauth
            </div>
        </div>
                ...
                <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse" style="background-color: #f0f0f0;">
                    <div class="navbar-nav m-auto py-0" style="padding: 10px 20px; background-color: #f0f0f0; border-right: 1px solid #ffff; {{ couleur_text_3() }};">
                        ...
                        @if($i <= 8) @if(sous_categories_menu($item->id)->count() == 0)
                            <a href="index.html" class="nav-item nav-link active" style="border-right: 1px solid #ffff; padding-right: 40px; {{ couleur_text_3() }};">{{ $item->nom }}</a>
                            @else
